...
export function TrendingActivities() {
  ...
  const totalMobilePages = Math.ceil(adventures.length / mobileItemsPerPage)
  ...
  const currentMobileAdventures = adventures.slice(
    mobileIndex * mobileItemsPerPage,
    (mobileIndex + 1) * mobileItemsPerPage
  )
  ...
  const nextMobileSlide = () => {
    setMobileIndex((prev) => (prev + 1) % totalMobilePages)
  }

  const prevMobileSlide = () => {
    setMobileIndex((prev) => (prev - 1 + totalMobilePages) % totalMobilePages)
  }
  ...
  return (
    <section className="w-full  bg-white">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-900">Top 10 Trending Adventure</h2>
        ...
        <div className="flex md:hidden gap-2">
          ...
          <button
            ...
            className="p-2 rounded-full border border-gray-300 hover:border-gray-400 transition-colors"
            disabled={mobileIndex === totalMobilePages - 1}
          >
            <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
      ...
      <div className="hidden md:grid md:grid-cols-3 lg:grid-cols-5 gap-4">
        {currentAdventures.map((adventure) => {
          const discountPercentage = Math.round(
            ((adventure.originalPrice - adventure.currentPrice) / adventure.originalPrice) * 100
          )
...
<h3 className="font-semibold text-gray-900 text-l mb-1">{adventure.title}</h3>
      ...
      {/* Mobile Grid - shows single card */}
      <div className="md:hidden">
        {currentMobileAdventures.map((adventure) => {
          const discountPercentage = Math.round(
            ((adventure.originalPrice - adventure.currentPrice) / adventure.originalPrice) * 100
          )
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  i === currentIndex ? 'bg-rose-600' : 'bg-gray-300'
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex gap-2">
            {Array.from({ length: totalMobilePages }, (_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  i === mobileIndex ? 'bg-rose-600' : 'bg-gray-300'
                }`}
              />
            ))}
          </div>
